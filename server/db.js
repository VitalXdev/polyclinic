...
const insertDoctorData = async (userId, contactId) => {
...
const verifyOTP = async (phoneNumber, otp) => {
  ...
  if (rows.length > 0 && rows[0].otp_sent === otp) {
    const updateQuery =
      "UPDATE otp SET validated = TRUE WHERE mobile_number = $1";
...
const updateDoctorQRCode = async (doctorId, qrCodeUrl) => {
  await pool.query("UPDATE Doctor SET qr_code_url = $1 WHERE doctor_id = $2", [
    qrCodeUrl,
    doctorId,
  ]);
};
...
// Query to insert NewClinic Details
const insertclinicDetails = async (clinic_name) => {
  const res = await pool.query(
    "INSERT INTO Clinic (clinic_name) VALUES($1) RETURNING clinic_id",
    [clinic_name]
  );
...
const updateAppointmentStatus = async (appointmentId, newStatus) => {
  ...
  const query =
    "UPDATE appointment SET status = $1 WHERE appointment_number = $2";
